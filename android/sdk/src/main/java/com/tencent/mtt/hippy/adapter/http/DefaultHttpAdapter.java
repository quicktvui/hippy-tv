...
public class DefaultHttpAdapter implements HippyHttpAdapter {
  ...
  private void execute(Runnable runnable) {
    if (mExecutorService == null) {
      int poolSize = Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ? 5 : 3;
  ...
  protected HttpURLConnection createUrlConnection(HippyHttpRequest request, URL url) throws Exception {
    configUrlConnection(request, url);
    if (!TextUtils.isEmpty(request.getProxyHostName()) && request.getProxyPort() != 0) {
      Proxy proxy = new Proxy(Proxy.Type.HTTP,
        new InetSocketAddress(request.getProxyHostName(), request.getProxyPort()));
