...
public class HippyTag {
  ...
  public final static String TAG_CLASS_CACHE = "cacheType";

  public static HippyMap createTagMap(String className, HippyMap iniProps,String cacheType) {
    HippyMap tagMap = new HippyMap();
    tagMap.pushString(TAG_CLASS_NAME, className);
    if(!TextUtils.isEmpty(cacheType)) {
      tagMap.pushString(TAG_CLASS_CACHE, cacheType);
    }
    return tagMap;
  }
