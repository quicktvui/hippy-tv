...
public class HippyRootView extends FocusDispatchView {
  ...
  public HippyRootView(HippyEngine.ModuleLoadParams loadParams) {
    ...
    //zhaopeng add 2023 11/14
//    if(getContext() instanceof HippyInstanceContext){
//      ((HippyInstanceContext) getContext()).getContextExt().setRootView(this);
//    }

    setId(mInstanceId);
    //setTag(NodeProps.ROOT_NODE);
    HippyMap tagMap = HippyTag.createTagMap(NodeProps.ROOT_NODE, null,null);
    setTag(tagMap);
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    if(mDownKeyEventEnabled){
      HippyModuleManager manager = mEngineContext.getModuleManager();
      HippyMap hippyMap = new HippyMap();
      hippyMap.pushInt("keyCode",event.getKeyCode());
      manager.getJavaScriptModule(EventDispatcher.class).receiveNativeEvent("onKeyDownEvent", hippyMap);
    }
    if(isKeyEventIntercept){
      return true;
    }
  ...
  public boolean onKeyUp(int keyCode, KeyEvent event) {
    if(mUpKeyEventEnabled){
      HippyModuleManager manager = mEngineContext.getModuleManager();
      HippyMap hippyMap = new HippyMap();
      hippyMap.pushInt("keyCode",event.getKeyCode());
      manager.getJavaScriptModule(EventDispatcher.class).receiveNativeEvent("onKeyUpEvent", hippyMap);
    }
    if(isKeyEventIntercept){
      return true;
    }
  ...
  public boolean dispatchKeyEvent(KeyEvent event) {
    if(dispatchKeyEventEnabled){
      HippyModuleManager manager = mEngineContext.getModuleManager();
      HippyMap hippyMap = new HippyMap();
      hippyMap.pushInt("action",event.getAction());
      hippyMap.pushInt("keyCode",event.getKeyCode());
      hippyMap.pushInt("keyRepeat", event.getRepeatCount());
      manager.getJavaScriptModule(EventDispatcher.class).receiveNativeEvent("onDispatchKeyEvent", hippyMap);
    }
    if(isKeyEventIntercept){
      return true;
    }
  ...
  private boolean dispatchKeyEventEnabled = false;
  public void enableDispatchEvent(boolean enable){
    this.dispatchKeyEventEnabled = enable;
  }

  private boolean mDownKeyEventEnabled = false;
  public void enableKeyDownEvent(boolean enable) {
    mDownKeyEventEnabled = enable;
  }

  private boolean mUpKeyEventEnabled = false;
  public void enableKeyUpEvent(boolean enable) {
    mUpKeyEventEnabled = enable;
  }

  private boolean isKeyEventIntercept = false;

  public void setKeyEventIntercept(boolean keyIntercept) {
    isKeyEventIntercept = keyIntercept;
  }
}
