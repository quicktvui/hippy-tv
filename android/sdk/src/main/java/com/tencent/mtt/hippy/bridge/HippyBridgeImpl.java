...
//    }
  ...
  private HippyModuleManager hippyModuleManager;
  ...
  public HippyBridgeImpl(HippyEngineContext engineContext, BridgeCallback callback,
    ...
    hippyModuleManager = mContext.getModuleManager();
  ...
  public void destroy(NativeCallback callback) {
    if(LogUtils.isDebug()){
      LogUtils.d(TAG, "before destroy");
    }
    destroy(mV8RuntimeId, mSingleThreadMode, callback);
    if(LogUtils.isDebug()){
      LogUtils.d(TAG, "after destroy");
    }
  }
  ...
  public void fetchResourceWithUri(final String uri, final long resId) {
    UIThreadUtils.runOnUiThread(new Runnable() {
      ...
      public void run() {
        ...
        ByteBuffer buffer = DebugCache.get().restore(uri);
        if(buffer != null) {
        ...
        devManager.loadRemoteResource(uri, new DevServerCallBack() {
          ...
          public void onDevBundleLoadReady(InputStream inputStream) {
            try {
              ...
              buffer.put(resBytes);
              onResourceReady(buffer, mV8RuntimeId, resId);
              DebugCache.get().cache(uri, buffer);
            } catch (Throwable e) {
              if (mBridgeCallback != null) {
                mBridgeCallback.reportException(e);
              }
...
//      LogUtils.d("hippy_bridge", "bytesToArgument using Buffer");
  ...
  public void reportException(String message, String stackTrace) {
    ...
    if (hippyModuleManager !=null){
      ...
      hippyMap.pushString("stackTrace",stackTrace);
      hippyModuleManager.getJavaScriptModule(EventDispatcher.class).receiveNativeEvent("onJsErrorException", hippyMap);
    }
    if (mBridgeCallback != null) {
