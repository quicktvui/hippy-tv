...
//                    }
          ...
          if (overPullListener != null) {
            overPullListener
                .onOverPullStateChanged(overPullState, overPullState, getOverPullOffset());
          }
        } else {
   ...
   * 等到onScrollStateChanged 变成 IDLE的时候，再做rollbackToBottomOrTop 2、如果当前处于非fling状态，scrollState的值不是SCROLL_STATE_SETTLING，就立即做rollbackToBottomOrTop
   */
  ...
  private boolean isOverPullUp(MotionEvent event) {
    ...
    int distanceToBottom =
        recyclerView.computeVerticalScrollOffset() + recyclerView.getHeight() - recyclerView
