...
class WebSocket {
  ...
  constructor(url, protocols, extrasHeaders) {
    ...
    Native.callNativeWithPromise(WEB_SOCKET_MODULE_NAME, 'connect', params).then((resp) => {
      ...
      console.log('websocket', 'called connect error ' + error)
    });
  }
  ...
  close(code, reason) {
    if (this.readyState !== READY_STATE_OPEN) {
      ...
      console.log('websocket', 'mark close ' + this.webSocketId)
      return;
    }

    console.log('websocket', 'to close')
  ...
  onWebSocketEvent(param) {
    ...
    if (eventType === 'onOpen') {
      ...
      if(this.isMarkClose) {
        console.log('websocket', 'close with mark ' + this.webSocketId)
        this.close()
      }
    } else if (eventType === 'onClose') {
      this.readyState = READY_STATE_CLOSED;
      // Vue.prototype.websocketOnTime--;
      // console.log('websocket', ' before close - ' +  Vue.prototype.websocketOnTime);
      // if(Vue.prototype.websocketOnTime <= 0) {
      //   Vue.prototype.websocketOnTime = 0;
